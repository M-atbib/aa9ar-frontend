...
export default function ProjectList({ isLoading }: { isLoading: boolean }) {
  ...
  const { projects, partnershipProjects, getPartnershipProjects } =
    useProjectStore();

  useEffect(() => {
    getPartnershipProjects(); // Fetch partnership projects on component mount
  }, [getPartnershipProjects]);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Partnership Projects Section */}
      <div>
        <h2 className="text-lg font-bold mb-4">Partnership Projects</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {partnershipProjects.length > 0
            ? partnershipProjects.map((project) => (
              <Card key={project.id} className="bg-primaryDark/90 text-white">
                <CardHeader className="flex flex-row items-center justify-between relative">
                  <div>
                    <CardTitle>{project.name}</CardTitle>
                    <CardDescription>{project.address}</CardDescription>
                ...
                <CardContent>
                 ...
                 <CardFooter className="flex items-end justify-end mb-0">
                  <Link href={`/project/${project.id}`}>
                    <Button className="w-full" variant="secondary">
                      Inspecter les Dossiers
                    </Button>
            ...
            : "No Partnership Projects available"}
        </div>
      </div>
